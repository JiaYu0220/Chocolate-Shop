<template>
  <FormModal target="order" ref="orderFormModal" @reset-form="resetModalForm">
    <!-- 標題 -->
    ...
    <template v-if="Object.keys(tempOrder).length !== 0">
      <!-- 編輯區 -->
      <VForm v-slot="{ errors }" ref="orderForm" @submit="modalPutOrder">
        <h4 class="mb-3 text-xl font-bold text-primary-800">訂單資訊</h4>
        <div class="mb-3 flex gap-3">
          ...
          <div class="w-1/2">
            ...
            <p class="text-primary-800" v-if="coupon">使用優惠券 {{ coupon }}</p>
          </div>
        </div>
        <div class="mb-6">
          <!-- 付款狀態 -->
        ...
        <h4 class="mb-3 text-xl font-bold text-primary-800">客戶資訊</h4>
        <div class="mb-6 flex flex-col gap-3">
          ...
          <h4 class="mb-3 text-xl font-bold text-primary-800">訂單產品</h4>
          <TableComponent class="mb-8">
            <template #th>
              <th scope="col">產品</th>
              <th scope="col" class="min-w-10">數量</th>
              <th scope="col" class="min-w-8 text-end">小計</th>
            </template>
            <template #tr="{ trClass }">
              <tr :class="trClass" v-for="product in order.products" :key="product.id">
                <td>
                  <div class="flex items-center gap-2">
                    <div class="h-9 w-12 overflow-hidden rounded-sm">
                ...
                <td>
                  ...
                  >
                    <option v-for="i in 20" :key="`${product.id}${i}`" :value="i">
                      {{ i }}
                    </option>
                  </select>
                </td>
